...
def _run_embedding_log_test_process(rootdir, testname) -> subprocess.Popen:
    ...
    p = subprocess.Popen([sys.executable, embedded_py, testname], stderr=subprocess.PIPE)
    p.wait()
    return p


def _run_embedding_log_test(rootdir, testname) -> typing.Tuple[str, str]:
    ...
    Returns the stderr and returncode
    """
...
def test_logging_write_log_before_init(rootdir):
    ...
    stderr, returncode = _run_embedding_log_test(rootdir, "log_before_initialize")
    assert returncode != 0
    assert (
...
def test_logging_write_info_after_initialize_with_error_level(rootdir):
    ...
    stderr, returncode = _run_embedding_log_test(

...
def test_qk_eng_wb2_005(printer):
    ...
    def innertest():
        ...
        printer("Apply loads")
        selection.UpdateSelection(ExtAPI, [26], Ansys.ACT.Interfaces.Common.SelectionTypeEnum.GeometryEntities)
        FIX_SUP = STAT_STRUC.AddFixedSupport()
        selection.UpdateSelection(ExtAPI, [25], Ansys.ACT.Interfaces.Common.SelectionTypeEnum.GeometryEntities)
        ...
        assert d1_mode1 == pytest.approx(37.268, rel=.05), "First Load Multiplier result"
        d1_mode3 = TOT_DEF01_BUCK.TabularData["LoadMultiplier"].get_Item(2)
        assert d1_mode3 == pytest.approx( 335.412, rel=.05), "Third Load Multiplier result"
        d1_mode5 = TOT_DEF01_BUCK.TabularData["LoadMultiplier"].get_Item(4)
        assert d1_mode5 == pytest.approx(931.701, rel=.05), "Fifth Load Multiplier result"
...
def test_qk_eng_wb2_007(printer):
    ...
    def innertest():
        ...
        selection.UpdateSelection(ExtAPI, [29], Ansys.ACT.Interfaces.Common.SelectionTypeEnum.GeometryEntities)
        FIX_SUP = STAT_STRUC1.AddFixedSupport()

        ExtAPI.SelectionManager.ClearSelection()
        selection.UpdateSelection(ExtAPI, [28], Ansys.ACT.Interfaces.Common.SelectionTypeEnum.GeometryEntities)
        ...
        SHEAR_STRS1_STAT_STRUC1.ShearOrientation = Ansys.Mechanical.DataModel.Enums.ShearOrientationType.YZPlane
        SHEAR_STRS2_STAT_STRUC1 = SOLN_STAT_STRUC1.AddShearStress()
        SHEAR_STRS2_STAT_STRUC1.ShearOrientation = Ansys.Mechanical.DataModel.Enums.ShearOrientationType.XZPlane
        ...
        SHEAR_STRS1_STAT_STRUC2.ShearOrientation = Ansys.Mechanical.DataModel.Enums.ShearOrientationType.YZPlane
        ...
        FAT_TOOL1.StressComponent = Ansys.Mechanical.DataModel.Enums.FatigueStressComponentType.FatigueToolComponent_YZ
        ...
        FAT_TOOL2.StressComponent = Ansys.Mechanical.DataModel.Enums.FatigueStressComponentType.FatigueToolComponent_XZ

        printer("Solve and Validate Fatigue Tool results")
        MODEL.Solve(True)

        FAT_Tool_1_LIFE = LIFE.Minimum.Value
        assert FAT_Tool_1_LIFE == pytest.approx(1628.0823, rel=.08), "Minimum Life For Fatigue Tool 1"

        FAT_TOOL_1_DAMAGE = DAMAGE.Maximum.Value
        assert FAT_TOOL_1_DAMAGE == pytest.approx(614.2195, rel=.08), "Maximum Damage For Fatigue Tool 1"

        FAT_TOOL_1_Safety_Factor = safetyFactor.Minimum.Value
        assert FAT_TOOL_1_Safety_Factor == pytest.approx(0.0122, rel=.08), "Minimum Safety Factor For Fatigue Tool 1"

        FAT_TOOL_2_LIFE = FAT_TOOL2.Children[0].Minimum.Value
        assert FAT_TOOL_2_LIFE == pytest.approx(1628.0823, rel=.08), "Minimum Life For Fatigue Tool 2"

        FAT_TOOL_2_DAMAGE = FAT_TOOL2.Children[1].Maximum.Value
        assert FAT_TOOL_2_DAMAGE == pytest.approx(614.2195, rel=.08), "Maximum Damage For Fatigue Tool 2"

        FAT_TOOL_2_Safety_Factor = FAT_TOOL2.Children[2].Minimum.Value
        assert FAT_TOOL_2_Safety_Factor == pytest.approx(0.0122, rel=.08), "Minimum Safety Factor For Fatigue Tool 2"
    innertest()
